...
var crawler = function(options) {
    ...
    self.queue = function (item, expiration, callback) {


      if(typeof(expiration) !== "undefined") {
        self.options.expiration = expiration;
      } else {
        self.options.expiration = 0;
      }
    ...
    self.checkIfFileExists = function (opts, callback, staticFilename) {
      ...
      if(self.options.expiration != 0) {
        var valueincache = cache.get(staticFilename);
        if(valueincache != null) {
          callback(null, valueincache);
        } else {
          callback();
        }
      } else {
    ...
    self.writeStaticCache = function (uri, response, callback) {
      ...
      if(self.options.expiration != 0) {
        cache.put(staticFilename, response.body, self.options.expiration);
        callback();
      } else if(self.options.loadstatic == true) {
    ...
    self.getfilename = function (href, callback) {
      ...
      self.createIfNotExists(urlparsed.hostname, datestamp, function () {
        callback();
      });
    };
